...
votes_metric = prometheus_client.Gauge("aptos_validator_votes", "", ["peer_id"])
...
def get_data():
    data = []
    try:
        response = requests.get("https://community.aptoslabs.com/it2", timeout=30)
    ...
    for item in tr_list:
        ...
        peer_id = item_children[1].text.strip("\n").strip(" ")

        liveness_content = item_children[2].text_content()
        liveness = liveness_content.split("%")[0].rsplit(" ", 1)[1]
        liveness = float(liveness)

        participation_content = item_children[3].text_content()
        participation = participation_content.split("%")[0].rsplit(" ", 1)[1]
        participation = float(participation)

        votes_content = item_children[4].text_content()
        votes_content = votes_content.strip("\n")
        votes_content = votes_content.strip(" ")
        votes = int(votes_content)
...
def update_metrics():
    ...
    for item in item_list:
        ...
        votes_metric.labels(item["peer_id"]).set(item["votes"])
